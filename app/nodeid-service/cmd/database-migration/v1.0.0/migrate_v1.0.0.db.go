...
import (
	"context"
	dbv1_0_0_nodeid "github.com/go-micro-saas/nodeid-service/app/nodeid-service/cmd/database-migration/v1.0.0/nodeid"
...
func Upgrade(ctx context.Context, dbConn *gorm.DB, migrateRepo migrationpkg.MigrateRepo) (err error) {
	...
	// dbv1_0_0_nodeid
	err = dbv1_0_0_nodeid.NewMigrateHandler(dbConn, migrateRepo).Upgrade(ctx)
	if err != nil {
		return err
	}
